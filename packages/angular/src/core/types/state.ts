...
export type IngKey = 'dragging' | 'wheeling' | 'moving' | 'hovering' | 'scrolling' | 'pinching' | 'pullingToRefresh';
...
export type SharedGestureState = {
  ...
  pinching?: boolean;
  /**
   * True if the element is being pulled to refresh.
   */
  pullingToRefresh?: boolean;
  /**
...
export interface DragState extends CoordinatesState {
  _pointerId?: number;
...
export interface PullToRefreshState extends CoordinatesState {
  /**
   * True when the pull-to-refresh gesture has been canceled by the `cancel` function.
   */
  canceled: boolean;
  /**
   * Function that can be called to cancel the pull-to-refresh.
   */
  cancel(): void;
  /**
   * True when the pull distance has reached the threshold to trigger a refresh.
   */
  shouldRefresh: boolean;
  /**
   * The pull distance as a percentage of the threshold (0-100+).
   */
  progress: number;
}
...
export type EventTypes = {
  ...
  pullToRefresh: PointerEvent | TouchEvent;
};
...
export interface State {
  ...
  pullToRefresh?: PullToRefreshState & { event: EventTypes['pullToRefresh'] };
}
