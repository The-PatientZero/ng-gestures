...
export type UserHoverConfig = GenericOptions & MoveAndHoverMouseOnly;

export type PullToRefreshConfig = GestureOptions<'pullToRefresh'> & {
  /**
   * The distance (in pixels) needed to trigger a pull-to-refresh.
   */
  threshold?: number;
  /**
   * The maximum distance (in pixels) the pull can extend.
   */
  maxDistance?: number;
  /**
   * The resistance factor when pulling beyond the threshold.
   * Higher values make it harder to pull.
   */
  resistance?: number;
  /**
  ...
  /**
   * The axis that the pull-to-refresh gesture should be detected on.
   */
  axis?: 'y';
};

export type UserPullToRefreshConfig = GenericOptions & PullToRefreshConfig;
...
export type UserGestureConfig = GenericOptions & {
  ...
  move?: MoveConfig;
  pinch?: PinchConfig;
  hover?: { enabled?: boolean } & MoveAndHoverMouseOnly;
