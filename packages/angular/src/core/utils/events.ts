...
function hasCapture(actionKey: string, capture = false) {
...
export function toHandlerProp(device: string, action = '', capture: boolean = false) {
  ...
  return 'on' + capitalize(device) + capitalize(actionKey) + (hasCapture(actionKey, capture) ? 'Capture' : '');
}
...
function getValueEvent<EventType extends TouchEvent | PointerEvent>(
  ...
  return (isTouch(event) ? getTouchList(event)[0] : event) as any;
}
...
export function distanceAngle(P1: Touch | PointerEvent, P2: Touch | PointerEvent) {
  try {
    ...
    // Ignore errors in pinch calculation
  }
  return null;
}
